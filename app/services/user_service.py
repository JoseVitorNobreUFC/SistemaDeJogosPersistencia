...
async def create(db: AsyncSession, payload: Dict[str, Any]):
    try:
        ...
        logger.info_("User criado")
        return obj
    except IntegrityError as e:
        await db.rollback()
        error_msg = str(e.orig).lower()
        if "email" in error_msg or "unique constraint" in error_msg:
            logger.error_(f"Tentativa de criar usuário com email duplicado: {payload.get('email')}")
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT, 
                detail="Já existe um usuário com este email"
            )
        logger.error_(f"Erro de integridade ao criar usuário: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Erro de integridade nos dados fornecidos"
        )
    except ValueError as e:
        logger.error_(f"Erro de validação: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail=str(e)
        )
    except Exception as e:
        logger.error_(f"Erro inesperado ao criar usuário: {str(e)}")
        raise HTTPException(
...
async def get(db: AsyncSession, user_id: int):
    ...
    if not obj:
        logger.error_("User não encontrado")
...
async def update(db: AsyncSession, user_id: int, payload: Dict[str, Any]):
    ...
    try:
        ...
        logger.info_("User atualizado")
        return {"message": "User atualizado com sucesso"}
    except IntegrityError as e:
        await db.rollback()
        error_msg = str(e.orig).lower()
        if "email" in error_msg or "unique constraint" in error_msg:
            logger.error_(f"Tentativa de atualizar usuário para email duplicado: {payload.get('email')}")
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT, 
                detail="Já existe um usuário com este email"
            )
