...
async def create(db: AsyncSession, payload: Dict[str, Any]):
    try:
        ...
        if "jogo_id" in str(e.orig) and "not present in table" in str(e.orig):
            logger.error_(f"Tentativa de criar review para jogo inexistente: {payload.get('jogo_id')}")
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, 
                detail="Não existe um jogo com este id"
            )
        elif "usuario_id" in str(e.orig) and "not present in table" in str(e.orig):
            logger.error_(f"Tentativa de criar review para usuário inexistente: {payload.get('usuario_id')}")
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, 
                detail="Não existe um usuário com este id"
            )
        logger.error_(f"Erro de integridade ao criar review: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
...
async def get(db: AsyncSession, review_id: int):
    ...
    if not obj:
        logger.error_("Review não encontrada")
...
async def update(db: AsyncSession, review_id: int, payload: Dict[str, Any]):
    ...
    try:
        ...
        logger.info_("Review atualizada")
        return {"message": "Review atualizada com sucesso"}
    except IntegrityError as e:
        await db.rollback()
        if "jogo_id" in str(e.orig) and "not present in table" in str(e.orig):
            logger.error_(f"Tentativa de atualizar review para jogo inexistente: {payload.get('jogo_id')}")
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, 
                detail="Não existe um jogo com este id"
            )
        elif "usuario_id" in str(e.orig) and "not present in table" in str(e.orig):
            logger.error_(f"Tentativa de atualizar review para usuário inexistente: {payload.get('usuario_id')}")
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, 
                detail="Não existe um usuário com este id"
            )
        logger.error_(f"Erro de integridade ao atualizar review: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Erro de integridade nos dados"
        )
