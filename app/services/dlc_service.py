...
async def create(db: AsyncSession, data: Dict[str, Any]):
  try:
    ...
    logger.info_("DLC criado")
    return obj
  except IntegrityError as e:
    if "titulo" in str(e.orig):
      logger.error_(f"Tentativa de criar DLC com título duplicado: {data.get('titulo')}")
      raise HTTPException(
          status_code=status.HTTP_409_CONFLICT, 
          detail="Já existe um DLC com este título"
      )
    if "jogo_id" in str(e.orig) and "not present in table" in str(e.orig):
      logger.error_(f"Tentativa de criar DLC para jogo inexistente: {data.get('jogo_id')}")
      raise HTTPException(
